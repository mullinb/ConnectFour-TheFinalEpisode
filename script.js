...
$(document).ready(function(){
    ...
    var string = '<div class="options"><div class="option1 name"></div><div class="option1 left"></div><div class="option1 value"></div><div class="option1 right"></div><div class="option2 name"></div><div class="option2 left"></div><div class="option2 value"></div><div class="option2 right"></div><div class="option3 name"></div><div class="option3 left"></div><div class="option3 value"></div><div class="option3 right"></div><div class="option4 name"></div><div class="option4 left"></div><div class="option4 value"></div><div class="option4 right"></div><div class="startDialog"></div></div>'
...
//====================GAME STATE VARIABLES=================//
    ...
    function generateBoardAndDataTrackers() {
        ...
        if (firstGameCounter == true){
             $("body").append("<div id='bodydiv'><div id='playzone'></div><div id='gameboard'></div></div>")
        } else {
            $(".slots").detach();
            $(".squares").detach();
            $(".red").remove();
...
//=====================================ENDGAME CONTENT================================///
   ...
   (function prepareEndgameContent() {
       ...
       $('.endgameDecision').append("<div class='winBanner'></div><div class='deathRebirth'></div><div class='accessInstrumentality'></div>" )
   ...
   function winConditions(color, i, j){
       if (option4val === 4) {
           try {
               if ($(dataArray[i][j+1]).is(color) && $(dataArray[i][j+2]).is(color) && $(dataArray[i][j+3]).is(color)) {
                   coin.pause();
                   gameEnding = true;
                   happening = true;
                   setTimeout(function() {
               ...
               } else if ($(dataArray[i+1][j]).is(color) && $(dataArray[i+2][j]).is(color) && $(dataArray[i+3][j]).is(color)) {
                   coin.pause();
                   gameEnding = true;
                   happening = true;
                   setTimeout(function() {
               ...
               } else if ($(dataArray[i+1][j+1]).is(color) && $(dataArray[i+2][j+2]).is(color) && $(dataArray[i+3][j+3]).is(color)) {
                   coin.pause();
                   gameEnding = true;
                   happening = true;
                   setTimeout(function() {
            ...
            try {
                if ($(dataArray[i][j+1]).is(color) && $(dataArray[i][j+2]).is(color)) {
                    coin.pause();
                    gameEnding = true;
                    happening = true;
                    setTimeout(function() {
                ...
                } else if ($(dataArray[i+1][j]).is(color) && $(dataArray[i+2][j]).is(color)) {
                    coin.pause();
                    gameEnding = true;
                    happening = true;
                    setTimeout(function() {
                ...
                } else if ($(dataArray[i+1][j+1]).is(color) && $(dataArray[i+2][j+2]).is(color)) {
                    coin.pause();
                    gameEnding = true;
                    happening = true;
                    setTimeout(function() {
             ...
             try {
                 if ($(dataArray[i][j+1]).is(color) && $(dataArray[i][j+2]).is(color) && $(dataArray[i][j+3]).is(color) && $(dataArray[i][j+4]).is(color)) {
                     coin.pause();
                     gameEnding = true;
                     happening = true;
                     setTimeout(function() {
                 ...
                 } else if ($(dataArray[i+1][j]).is(color) && $(dataArray[i+2][j]).is(color) && $(dataArray[i+3][j]).is(color) && $(dataArray[i+4][j]).is(color)) {
                     coin.pause();
                     gameEnding = true;
                     happening = true;
                     setTimeout(function() {
                 ...
                 } else if ($(dataArray[i+1][j+1]).is(color) && $(dataArray[i+2][j+2]).is(color) && $(dataArray[i+3][j+3]).is(color) && $(dataArray[i+4][j+4]).is(color)) {
                     coin.pause();
                     gameEnding = true;
                     happening = true;
                     setTimeout(function() {
              ...
              try {
                  if ($(dataArray[i][j+1]).is(color) && $(dataArray[i][j+2]).is(color) && $(dataArray[i][j+3]).is(color) && $(dataArray[i][j+4]).is(color) && $(dataArray[i][j+5]).is(color)) {
                      coin.pause();
                      gameEnding = true;
                      happening = true;
                      setTimeout(function() {
                  ...
                  } else if ($(dataArray[i+1][j]).is(color) && $(dataArray[i+2][j]).is(color) && $(dataArray[i+3][j]).is(color) && $(dataArray[i+4][j]).is(color) && $(dataArray[i+5][j]).is(color)) {
                      coin.pause();
                      gameEnding = true;
                      happening = true;
                      setTimeout(function() {
                  ...
                  } else if ($(dataArray[i+1][j+1]).is(color) && $(dataArray[i+2][j+2]).is(color) && $(dataArray[i+3][j+3]).is(color) && $(dataArray[i+4][j+4]).is(color) && $(dataArray[i+5][j+5]).is(color)) {
                      coin.pause();
                      gameEnding = true;
                      happening = true;
                      setTimeout(function() {
               ...
               try {
                   if ($(dataArray[i][j+1]).is(color) && $(dataArray[i][j+2]).is(color) && $(dataArray[i][j+3]).is(color) && $(dataArray[i][j+4]).is(color) && $(dataArray[i][j+5]).is(color) && $(dataArray[i][j+6]).is(color)) {
                       coin.pause();
                       gameEnding = true;
                       happening = true;
                       setTimeout(function() {
                   ...
                   } else if ($(dataArray[i+1][j]).is(color) && $(dataArray[i+2][j]).is(color) && $(dataArray[i+3][j]).is(color) && $(dataArray[i+4][j]).is(color) && $(dataArray[i+5][j]).is(color) && $(dataArray[i+6][j]).is(color)) {
                       coin.pause();
                       gameEnding = true;
                       happening = true;
                       setTimeout(function() {
                   ...
                   } else if ($(dataArray[i+1][j+1]).is(color) && $(dataArray[i+2][j+2]).is(color) && $(dataArray[i+3][j+3]).is(color) && $(dataArray[i+4][j+4]).is(color) && $(dataArray[i+5][j+5]).is(color) && $(dataArray[i+6][j+6]).is(color)) {
                       coin.pause();
                       gameEnding = true;
                       happening = true;
                       setTimeout(function() {
...
//===============COMPUTER PLAYER FUNCTION =====================================================//
    ...
    function moveEvaluator(move, iteration) {
        ...
        for (var i=0; i<option4val; i++) {
            try {
                if ($(dataArray[thisMove.row-1][thisMove.column]).is(".yellow")) {                 ////CHECKING QUALITY AGAINST COINS BELOW
            ...
            try {
                if ($(dataArray[thisMove.row-1][thisMove.column-1]).is(".yellow")) {          ////CHECKING QUALITY AGAINST COINS BOTTOM LEFT DIAGONAL
                    thisMove.firstVal += 10;
                    try {
            ...
            try {
                if ($(dataArray[thisMove.row][thisMove.column-1]).is(".yellow")) {           ////CHECKING QUALITY AGAINST COINS TO THE LEFT
                    thisMove.firstVal += 10;
                    try {
            ...
            try {
                if ($(dataArray[thisMove.row+1][thisMove.column-1]).is(".yellow")) { ////CHECKING QUALITY AGAINST COINS TOP LEFT DIAGONAL
                    thisMove.firstVal += 10;
                    try {
                        if ($(dataArray[thisMove.row+2][thisMove.column-2]).is(".yellow")) {
                            thisMove.firstVal += 30;
            ...
            try {
                if ($(dataArray[thisMove.row+1][thisMove.column+1]).is(".yellow")) {    ////CHECKING QUALITY AGAINST COINS TOP RIGHT DIAGONAL
                    thisMove.firstVal += 10;
                    try {
                        if ($(dataArray[thisMove.row+2][thisMove.column+2]).is(".yellow")) {
                            thisMove.firstVal += 30;
            ...
            try {
                if ($(dataArray[thisMove.row][thisMove.column+1]).is(".yellow")) {      ////CHECKING QUALITY AGAINST COINS TO THE RIGHT
                    thisMove.firstVal += 10;
                    try {
                        if ($(dataArray[thisMove.row][thisMove.column+2]).is(".yellow")) {
                            thisMove.firstVal += 30;
            ...
            try {
                if ($(dataArray[thisMove.row-1][thisMove.column+1]).is(".yellow")) {       ////CHECKING QUALITY AGAINST COINS TO THE BOTTOM RIGHT DIAGONAL
                    thisMove.firstVal += 10;
                    try {
                        if ($(dataArray[thisMove.row-2][thisMove.column+2]).is(".yellow")) {
                            thisMove.firstVal += 30;
...
//=============================IN-GAME EVENT FUNCTIONS AND LISTENERS======================================//
    ...
    function setUpControlEvents(){
        $(document).on('mousedown.play', function (e){
            if (happening === false && e.target.classList.contains("slots") && e.target != document.querySelector(".playSlot")) {
                ...
                for (i=0; i<option3val; i++) {
                   if (e.target == slots[i]) {
                       if (isEven(turn)){
                           $("#gameboard").append("<div id='coin" + turn + "' data-column=" + i + " data-row=" + gProg['col'+i] + " class='red'></div>");
                       } else {
                           $("#gameboard").append("<div id='coin" + turn + "' data-column=" + i + " data-row=" + gProg['col'+i] + " class='yellow'></div>");
                       }
                       coin.currentTime = 0;
        ...
        $(document).on("click.pick", function(e){
            if (e.target === document.querySelector(".playSlot")) {
               $(document).on("mousemove.placement", function(e){
                   $(".ready").css({
                       zIndex: -1,
                       left: e.clientX-50-document.querySelector("#bodydiv").offsetLeft,
                       right: 'auto',
                       bottom: 'auto'
                   })
               })
        ...
        $(document).on("keydown.keys", function(e) {
               ...
               $("#playzone").children().each(function() {
                   if ($(this).children().hasClass("ready")) {
                       return false;
                   }
                   x++
               })
